...
unsigned char set_contrast(unsigned char current) {
	...
	int constrastLevel = (current % 3);

	lcd_write(LCD_C, 0b00100001); // extended instruction set

	if (constrastLevel == 0){
		lcd_write(LCD_C, 0x80 | LCD_LOW_CONTRAST); // Low contrast
	} else if (constrastLevel == 1){
		lcd_write(LCD_C, 0x80 | LCD_DEFAULT_CONTRAST); // Default contrast
	} else if (constrastLevel == 2){
		lcd_write(LCD_C, 0x80 | LCD_HIGH_CONTRAST); // High contrast
	}

	lcd_write(LCD_C, 0b00100000); // standard instruction set

	constrastLevel = ((constrastLevel + 1) % 3);

	// Return the new enumerated constrast value
	return constrastLevel;
}
...
unsigned char set_inversion(unsigned char current) {
	...
	if (current == 0){
		lcd_write(LCD_C, 0b000001101);

		return 1;

	} else {
		lcd_write(LCD_C, 0b000001100);
